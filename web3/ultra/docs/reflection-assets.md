...
GLB/glTF carry standard metallic-roughness PBR, normal maps and available glTF IOR/clearcoat extensions. FBX transfers mesh UVs and common material channels; receiving engines may need manual IOR and clearcoat assignments. World-space wetness modulation, local environment capture and camera-dependent planar reflection are web renderer features and must be implemented in the destination engine. Architecture and props retain their placement, and paving UVMap islands and intentional module repetition are unchanged. The later [paving terrain pass](paving-terrain.md) tilts and settles the stones towards the pools, levels the water at 0.166 m and lowers the two supporting ground modules.
