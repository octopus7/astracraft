...
import { restore, persist } from '../navigation.js';
...
scene.fog = new THREE.FogExp2('#ead8e4', .004);
...
const post=postProcess(renderer);
let water;

const camera = new THREE.OrthographicCamera(-14, 14, 10, -10, .1, 120);
const home = { position: new THREE.Vector3(11, 19, 29), target: new THREE.Vector3(0, 1.2, 0), zoom: 1 };
...
const savedMode = restore(camera, controls);
document.querySelector('#auto-rotate').classList.toggle('active',controls.autoRotate);
document.querySelector('#auto-rotate').setAttribute('aria-pressed',String(controls.autoRotate));
persist(camera, controls, () => currentMode);
let toastTimer;
const modeText = {
...
function addPuddleReflection(root, detail) {
  if(water){water.add(root,detail);return;}
  let marker = null;
...
function setMode(mode, announce = true) {
  ...
  renderer.toneMappingExposure = mode === 'detail' ? .85 : .82;
...
function resize() {
  ...
  const span = aspect < .9 ? 26 / aspect : 20;
  ...
  renderer.setPixelRatio(Math.min(devicePixelRatio, 2));
  post.resize(Math.round(width*renderer.getPixelRatio()),Math.round(height*renderer.getPixelRatio()));
}
...
function animate() {
  ...
  water?.update(reducedMotion ? 0 : elapsed);
